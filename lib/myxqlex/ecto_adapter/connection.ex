# if Code.ensure_loaded?(MyXQLEx) do
    defmodule MyXQLEx.EctoAdapter.Connection do
      @moduledoc false
      ...
      def prepare_execute(conn, name, sql, params, opts) do
        MyXQLEx.prepare_execute(conn, name, sql, params, opts)
      end

      @impl true
      def query(conn, sql, params, opts) do
        opts = Keyword.put_new(opts, :query_type, :binary_then_text)
        MyXQLEx.query(conn, sql, params, opts)
      end

      @impl true
      def execute(conn, %{ref: ref} = query, params, opts) do
        case MyXQLEx.execute(conn, query, params, opts) do
          {:ok, %{ref: ^ref}, result} ->
            {:ok, result}
      ...
      def stream(conn, sql, params, opts) do
        MyXQLEx.stream(conn, sql, params, opts)
      end

      @impl true
      def to_constraints(%MyXQLEx.Error{mysqlex: %{name: :ER_DUP_ENTRY}, message: message}) do
        case :binary.split(message, " for key ") do
          [_, quoted] -> [unique: strip_quotes(quoted)]
          _ -> []
        end
      end
      def to_constraints(%MyXQLEx.Error{mysqlex: %{name: name}, message: message})
